...
static void aes_sw_Substitution(unsigned char *a, int aes_sw_mode)
{
	int i;
	for(i = 0; i < 16; i++){
		a[i] = (aes_sw_mode == 0) ? aes_sw_S[a[i]] : aes_sw_Si[a[i]];
	}
}
...
static void aes_sw_SwapRowCol(unsigned char *a)
{
	unsigned char t, i, j;
...
void aes_decrypt(unsigned char *Key, unsigned char *Data, unsigned char *Result)
{
    ...
    _rijndaelDecrypt(Result, aes_sw_k10);
}
...
void aes_att_encryption(u8 *key, u8 *plaintext, u8 *result)
{
	...
	_rijndaelSetKey(sk, aes_sw_k0, aes_sw_k10);

	for(i = 0; i < 16; i++){
		sk[i] = plaintext[15 - i];
	}
	
	_rijndaelEncrypt(sk, aes_sw_k0);
...
void aes_att_decryption(u8 *key, u8 *plaintext, u8 *result)
{
	...
	_rijndaelDecrypt(sk, aes_sw_k10);
