...
#endif /* ZBHCI_SWIRE_MODE */
#endif /* ZBHCI_USB_PRINT */
...
u8 usb_print_txMsg(u16 u16Type, u16 u16Length, u8 *pu8Data)
{
    ...
    *p++ = crc8;
    ...
    if (u16Length > sizeof(usb_print_outbuf) - 7) {
        u16Length = sizeof(usb_print_outbuf) - 7;
    }
...
void usb_print_task(void)
{
    ...
    if (msg->startFlag == ZBHCI_MSG_START_FLAG) {
        ...
        if (g_hci_cmd_handler) {
            g_hci_cmd_handler(msgType, pLen, msg->pData);
        }
#if 0
#if ZB_TEST_ENABLE
