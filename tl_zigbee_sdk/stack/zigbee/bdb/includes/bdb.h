...
/**
 * @brief Definition for Constants used by all nodes
 */
...
#define BDBC_MIN_FAST_POLLDURATION_AFTER_JOIN   (120)   //!< 30s a battery powered device remains in fast poll mode after successfully joining a network.

/**
 * @brief Definition for Constants used by all nodes supporting touch link
 */
...
/**
 * @brief Definition for trust center key exchange method
 */
enum {
...
/**
 * @brief Definition for link key type with which the node was able to
 *        decrypt the network key when the node joins a new network.
 */
enum {
...
/**
 * @brief Definition for Commissioning status.
 */
enum {
    ...
    BDB_COMMISSION_STA_ON_NETWORK,
    BDB_COMMISSION_STA_STEER_ERR,
    BDB_COMMISSION_STA_TOUCHLINK_FAILURE,

    BDB_LOSS_CONNECTIVITY,
};
...
/**
 * @brief Definition for bdb attribute identifier
 */
enum {
...
/**
 * @brief Definition for network type
 */
enum {
...
/**
 * @brief Definition for the role during commissioning
 */
enum {
...
/**
 * @brief Definition struct for bdb Attribute Identifier table
 */
typedef struct {
...
/**
 * @brief Definition for Commissioning Mode.
 */
typedef struct {
...
/**
 * @brief Definition for commissioning capabilities struct.
 */
typedef enum {
...
/**
 * @brief Definition for Attributes for bdb
 */
typedef struct {
...
/**
 * @brief Definition for commissioning settings for bdb
 */
typedef struct {
...
/**
 * @brief Definition for context for bdb
 */
typedef struct {
    ...
    union {
        ...
        ev_timer_event_t *retrieveTCLKTimer;
    };
 ...
 *
 * @param  simple_desc - the pointer to device's simple description
 * @param  setting     - bdb commissioning settings
 * @param  cb          - call back for bdb
 * @param  repower     - 1: real power-on, 0: from deep sleep
 *
...
/**
 * @brief  bdb default reporting configuration
 *
 * @param  endpoint
 * @param  profileID
 * @param  clusterID
 ...
 * @return    RET_OK or others
 */
u8 bdb_preInstallCodeAdd(addrExt_t ieeeAddr, u8 *pInstallCode);
...
/**
 * @brief  set supported key negotiation methods
 *
 * @param  bitmask
 *
 * @return none
 */
void bdb_keyNegotiationMethods(ss_suppKeyNegoBitmask_e bitmask);

/**
 * @brief  trust center link key update
 *
 * @param  conn_chk
 *
 * @return none
