...
/**
 * @brief The broadcast delivery time, in milliseconds
 *            nwkBroadcastDeliveryTime = 2 * nwkMaxDepth * ((0.05+(nwkcMaxBroadcastJitter / 2))
 *                                     + nwkPassiveAckTimeout * nwkBroadcastRetries / 1000)
 *
...
/**
 * @brief Size of the pan id translation table
 */
...
#define NWK_NLME_END_DEV_NEGO_REQ               0x91
#define NWK_NLME_END_DEV_NEGO_CNF               0x92
...
typedef enum {
    ...
    NWK_INTERNAL_NWK_COMMISSION_REQ_CMD_HANDLE,
    NWK_INTERNAL_NWK_COMMISSION_RSP_CMD_HANDLE,
...
/**
 * @brief NWK address allocation method.
 */
typedef enum {
...
typedef enum {
    ...
    NWK_STATUS_INVALID_INTERFACE        = 0xD5,
    NWK_STATUS_MISSING_TLV              = 0xD6,
    NWK_STATUS_INVALID_TLV              = 0xD7,
} zb_nwk_status_t;
 ...
 */
typedef enum {
    NWK_COMMAND_STATUS_NO_ROUTE_AVAILABLE               = 0x00, /*!< Previous spec. It shall be treated the same as 0x02. */
    NWK_COMMAND_STATUS_TREE_LINK_FAILURE                = 0x01, /*!< Previous spec. It shall be treated the same as 0x02. */
    NWK_COMMAND_STATUS_NONE_TREE_LINK_FAILURE           = 0x02, /*!< None-tree link failure */
    NWK_COMMAND_STATUS_LOW_BATTERY_LEVEL                = 0x03, /*!< Deprecated */
    NWK_COMMAND_STATUS_NO_ROUTING_CAPACITY              = 0x04, /*!< Deprecated */
    NWK_COMMAND_STATUS_NO_INDIRECT_CAPACITY             = 0x05, /*!< Deprecated */
    NWK_COMMAND_STATUS_INDIRECT_TRANSACTION_EXPIRY      = 0x06, /*!< Deprecated */
    NWK_COMMAND_STATUS_TARGET_DEVICE_UNAVAILABLE        = 0x07, /*!< Deprecated */
    NWK_COMMAND_STATUS_TARGET_ADDRESS_UNALLOCATED       = 0x08, /*!< Deprecated */
    NWK_COMMAND_STATUS_PARENT_LINK_FAILURE              = 0x09, /*!< Parent link failure */
    NWK_COMMAND_STATUS_VALIDATE_ROUTE                   = 0x0a, /*!< Deprecated */
    ...
    NWK_COMMAND_STATUS_VERIFY_ADDRESS                   = 0x0e, /*!< Deprecated */
    ...
    NWK_COMMAND_STATUS_BAD_FRAME_COUNTER                = 0x11, /*!< Deprecated */
    NWK_COMMAND_STATUS_BAD_KEY_SEQUENCE_NUMBER          = 0x12, /*!< Deprecated */
    NWK_COMMAND_STATUS_UNKNOWN_COMMAND                  = 0x13,
    NWK_COMMAND_STATUS_PAN_ID_CONFLICT_REPORT           = 0x14,
} nwk_statusCode_t;
...
typedef enum {
    JOIN_METHOD_ASSOCIATION                             = 0x00,
    JOIN_METHOD_REJOIN_INSECURITY                       = 0x01,
    JOIN_METHOD_REJOIN_SECURITY                         = 0x02,
    JOIN_METHOD_NWK_COMMISSION_JOIN_INSECURITY          = 0x03,
    JOIN_METHOD_NWK_COMMISSION_REJOIN_INSECURITY        = 0x04,
    JOIN_METHOD_NWK_COMMISSION_REJOIN_SECURITY          = 0x05,

    JOIN_METHOD_DIRECT_JOIN                             = 0xFF,
} join_method_t;
...
/**
 * @brief NLME-END-DEVICE-NEGOTIATE.request
 */
typedef struct {
    u8 *pEndDevTLVs;//TODO
    u8 tlvLen;
} nlme_endDevNegotiate_req_t;

/**
 * @brief NLME-END-DEVICE-NEGOTIATE.confirm
 */
typedef struct {
    u8 status;
} nlme_endDevNegotiate_cnf_t;
...
typedef enum {
    ...
    NWK_CMD_LINK_POWER_DELTA            = 0x0d,
    NWK_CMD_NETWORK_COMMISSION_REQ      = 0x0e,
    NWK_CMD_NETWORK_COMMISSION_RSP      = 0x0f,
} nwk_cmdId_t;
...
typedef enum {
    ...
    TIMEOUT_RSP_STATUS_UNSUPPORTED_FEATURE,
} endDevTimeoutRspStatus_t;

/**
 * @brief Value of the parent information bit mask
 */
typedef enum {
    MAC_DATA_POLL_KEEPALIVE_BIT         = BIT(0),
    END_DEV_TIMEOUT_REQ_KEEPALIVE_BIT   = BIT(1),
    POWER_NEGOTIATION_SUPPORT           = BIT(2),
    END_DEVICE_TLV_SUPPORT              = BIT(3),
...
/**
 * @brief Network commissioning type
 */
typedef enum {
    INITIAL_JOIN,
    REJOIN,
    ESTABLISH_TRUSTED_LINK, //Establish Trusted Link
} nwkCommissionType_t;

/**
 * @brief Network commissioning request command payload
 */
typedef struct {
    u8 *pZigbeeTLVs;
    u8 tlvLen;
    u8 type;//nwkCommissionType_t
    capability_info_t capabilityInfo;
} nwkCmd_nwkCommissionReq_t;

/**
 * @brief Network commissioning response command payload
 */
typedef struct {
    u16 nwkAddress;
    u8 status;
} nwkCmd_nwkCommissionRsp_t;
...
typedef struct {
    ...
    union {
        ...
        nwkCmd_nwkCommissionReq_t nwkCommissionReq;     /*!< NWK commissioning request command */
        nwkCmd_nwkCommissionRsp_t nwkCommissionRsp;     /*!< NWK commissioning response command */
    };
} nwkCmd_t;
...
/**
 * @brief Network broadcast entry status
 */
typedef enum {
    NWK_BRC_ENTRY_IDLE,
    NWK_BRC_ENTRY_NORMAL,
    NWK_BRC_ENTRY_SRC_ADDR_CONFLICT,
} nwk_brcStatus_t;
...
extern bool ZB_TRUST_ROUTE_BEHALF;
...
bool tl_zbNwkAddrUsedDetect(u16 nwkAddr, addrExt_t ieeeAddr);
...
void tl_zbNwkLinkStatusSendFirst(void);
...
void tl_zbNwkNlmeEndDevNegoRequestHandler(void *arg);

/**
 * @brief High layer to NWK layer primitive
 */
...
#define tl_zbNwkNlmeEndDevNegoRequest(p)        tl_zbPrimitivePost(TL_Q_HIGH2NWK, NWK_NLME_END_DEV_NEGO_REQ, p)
...
#if ZB_TRUST_LINK_SUPPORT
u8 tl_zbLinkTypeByShortAddr(u16 shortAddr, u8 *linkType);
u8 tl_zbLinkTypeByIeeeAddr(addrExt_t extAddr, u8 *linkType);

zb_nwk_status_t tl_zbTLinkRecvDataHandler(u8 *data, u8 dataLen, bool securityFlag);

typedef u8 (*nwkZbdPostproCb_t)(void *p, bool encrypt);
void tl_zbdPostproReqRegister(nwkZbdPostproCb_t cb);
#endif
