...
/**
 * @brief Startup control attribute
 */
typedef enum {
...
/**
 * @brief Network layer running state
 */
typedef enum {
    NLME_STATE_IDLE,
...
/**
 * @brief Place to store values between multiply operations
 */
typedef struct {
    ...
    u8 router_started:1;        /*!< Unused */
    u8 is_tc:1;                 /*!< True if we are Trust Center */
    u8 joined_pro:1;
    u8 panIdConflict:1;         /*!< If find panID conflict */
    u8 joinAccept:1;
    u8 known_panids_cnt:4;
    u8 discoverRoute:1;         /*!< Always TRUE for NLDE data request! see 2.2.4.1.1.3 */
    u8 panidUpdateRecv:1;       /*!< Receive network update for pan id conflict */
    u8 resv:1;
