...
typedef struct {
    ...
    u8 resv[3];
} nwk_nib_t; //60 or 48 bytes

#define BEACON_APPENDIX_TLVS_MAX_LEN    127

typedef struct {
#if (ZB_COORDINATOR_ROLE || ZB_ROUTER_ROLE)
    u8 wideBeaconTLVsLen;
    u8 wideBeaconAppendixTLVs[BEACON_APPENDIX_TLVS_MAX_LEN];

    u8 devBeaconTLVsLen;
    u8 devBeaconAppendixTLVs[BEACON_APPENDIX_TLVS_MAX_LEN];
#endif
    u32 nextChnChange;
    ...
    u8 performAddMacDataPollRetries;
    bool preferredParent;
    bool hubConnectivity;
    u8 goodParentLQA;

    u16 panIdConflictCnt;
    u8 maxInitJoinParentAttempts;
    u8 maxRejoinParentAttempts;
} nwk_nib_add_t;

extern nwk_nib_add_t g_nwkNIBAdd;
