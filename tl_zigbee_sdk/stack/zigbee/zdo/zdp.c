...
//  {USER_DESC_REQ_CLID,                FALSE,  zdo_userDescIndicate},          //DEPRECATED
...
#if (ZB_COORDINATOR_ROLE || ZB_ROUTER_ROLE)
    {MGMT_RTG_REQ_CLID,                 FALSE,  zdo_mgmtRtgIndicate},
#endif
    ...
    {CLEAR_ALL_BINDINGS_REQ_CLID,       TRUE,   zdo_clearAllBindIndicate},
    {MGMT_NWK_BEACON_SURVEY_REQ_CLID,   FALSE,  zdo_mgmtNwkBeaconSurveyIndicate},
#if ZB_COORDINATOR_ROLE
    {SEC_START_KEY_NEGOTIATION_REQ_CLID,FALSE,  zdo_secStartKeyNegoIndicate},
    {SEC_RETRIEVE_AUTH_TOKEN_REQ_CLID,  FALSE,  zdo_secRetrieveAuthTokenIndicate},
#endif
#if (ZB_ROUTER_ROLE || ZB_ED_ROLE)
    {SEC_START_KEY_UPDATE_REQ_CLID,     FALSE,  zdo_secStartKeyUpdateIndicate},
#endif
    {SEC_GET_AUTH_LEVEL_REQ_CLID,       FALSE,  zdo_secGetAuthLevelIndicate},
    {SEC_SET_CFG_REQ_CLID,              FALSE,  zdo_secSetConfigIndicate},
    {SEC_GET_CFG_REQ_CLID,              FALSE,  zdo_secGetConfigIndicate},
    {SEC_CHALLENGE_REQ_CLID,            FALSE,  zdo_secChallengeIndicate},
    {SEC_DECOMMISSION_REQ_CLID,         FALSE,  zdo_secDecommissionIndicate},
#endif
};
...
_CODE_ZDO_ void zdp_serverCmdHandler(void *ind)
{
...
#endif
    if (p->cluster_id == NWK_ADDR_RSP_CLID || p->cluster_id == IEEE_ADDR_RSP_CLID) {
        zdo_remoteAddrNotify(ind);
    }

    if (p->cluster_id == SEC_RETRIEVE_AUTH_TOKEN_RSP_CLID) {
        if ((p->src_short_addr != 0x0000) || !(p->security_status & SECURITY_IN_APSLAYER)) {
            zb_buf_free((zb_buf_t *)ind);
            return;
        }
    }
    ...
    zdpInd.zpdu = p->asdu;
    zdpInd.relayCmd = p->relayCmd;
    if (zdpInd.relayCmd) {
        ZB_64BIT_ADDR_COPY(zdpInd.joinerLongAddr, p->joinerLongAddr);
    }
...
_CODE_ZDO_ void zdo_complexDescIndicate(void *buf)
{
    ...
    u16 nwkAddrReq = BUILD_U16(payload[0], payload[1]);
    payload += 2;
