 ...
 *        APSDE-DATA indication primitive described in Zigbee Specification r18,
 *        2.2.4.1.3 APSDE-DATA.indication, page 29.
 */
...
typedef enum {
    ...
    ZDO_FRAME_TOO_LARGE                 = 0x90,
    ZDO_BAD_KEY_NEGOTIATION_METHOD      = 0x91,
    ZDO_TEMPORARY_FAILURE               = 0x92,

    ZDO_MISSING_TLV                     = 0xD6,//same as NWK
    ZDO_INVALID_TLV                     = 0xD7,//same as NWK
} zdo_status_t;
...
typedef bool (*zdo_nwkUpdateCb_t)(u16 newPanId);
...
typedef void (*zdo_nlmeEndDevNegoCnfCb_t)(nlme_endDevNegotiate_cnf_t *p);
...
typedef struct {
    ...
    zdo_nlmeEndDevNegoCnfCb_t zdoNlmeEndDevNegoCnfCb;   //only for ED
...
/**
 * @brief  The Config_Parent_Link_Retry_Thres hold is either created when the
 *         application is first loaded or initialized with a commissioning tool.
 *         It is used for the ZED to decide how many times it should retry to
 *         connect to the parent router before initiating the rejoin process.
 *
 * @param  none
 *
 ...
 * @param  none
 *
 * @return poll rate in millisecond
 */
...
/**
 * @brief  Interface for forgetting all trust link device.
 *
 * @param  none
 *
 * @return none
 */
void zdo_nlmeForgetAllTrustLink(void);
 ...
 * @return zdo_status_t
 */
zdo_status_t zdo_nlmeEndDevNegoReq(nlme_endDevNegotiate_req_t *pTLVs);

/**
 * @brief  Check if NWK manager is idle.
 *
 * @param  none
 *
