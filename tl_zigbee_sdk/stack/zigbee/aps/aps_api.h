...
#else
#define APS_TX_CACHE_TABLE_NUM                  8
#endif
...
#define APS_JOINER_TLVS_UNFRAGMENTED_MAX_SIZE   79

#define APS_FRAGMENT_CACHE_TABLE_NUM            1 //TODO
...
typedef enum {
    ...
    APS_STATUS_PEER_CANNOT_FRAGMENT     = 0xb1,
    APS_STATUS_UNKNOWN_FRAGMENT_SUPPORT = 0xb2,

    APS_STATUS_HANDLE_BACKING           = 0xb6,
    APS_STATUS_SHORT_ADDR_REQUESTING    = 0xb7,
...
typedef enum {
    ...
    APS_CMD_HANDLE_TRANSPORT_KEY_UNSOLICITED,
    ...
    APS_CMD_HANDLE_ACK,
    APS_CMD_HANDLE_RELAY_DOWNSTREAM,
    APS_CMD_HANDLE_RELAY_DOWNSTREAM_MAX = APS_CMD_HANDLE_RELAY_DOWNSTREAM + 5,
    APS_CMD_HANDLE_RELAY_UPSTREAM,
    APS_CMD_HANDLE_RELAY_UPSTREAM_MAX   = APS_CMD_HANDLE_RELAY_UPSTREAM + 5,
    APS_CMD_HANDLE_TXEPH_KEY,
} aps_handle_e;
...
typedef struct {
    ...
    u8 dst_ep;
    /*
     * The individual device address or group address to which
...
typedef struct {
    u16 dst_addr;       /* destination address */
 ...
 * @return none
 */
void tl_zbApsPibSet(u8 attrId, u8 len, u8 *value);
...
/**
 * @brief  Add binding information to the binding table
 *
 * @param  amr - bind request
 *
 * @return APS_STATUS_SUCCESS or others
 */
aps_status_t aps_me_bind_req(aps_me_bind_req_t *amr);

/**
 * @brief  Remove binding information from the binding table
 *
 * @param  amr - unbind request
 *
 * @return APS_STATUS_SUCCESS or others
 */
...
/**
 * @brief  Match target address information from the binding table
 *
 * @param  apsreq
 * @param  bindList
 *
...
/**
 * @brief  Replace the old address with the new one
 *
 * @param  extOrial - the extended address in the binding table
 * @param  extNew   - the new extended address to replace the old one
 *
 * @return none
 */
void aps_bindingTblDstExtSwapOut(addrExt_t extOrial, addrExt_t extNew);

/**
 * @brief  Group table initialization (restore the table from NV)
 *
 * @param  none
 *
 * @return NV_SUCC or others
 */
 ...
 *
 * @param  group_addr - the group address
 * @param  epNum      - endpoint number in this group address
 *
 ...
 * @param  aps_counter - the aps counter
 *
...
/**
 * @brief  Clean and stop internal clock
 *
 * @param  none
 *
 * @return none
...
/**
 * @brief  Data indication Register
 *
 * @param  cb - callback
 *
 * @return none
 ...
 *
 * @param  dataReq - pointer to the aps data request structure
 * @param  asdu    - pointer to the asdu
 * @param  length  - length of asdu
 *
...
typedef int (*app_trustLinkJoinStateRet_t)(u8 ret);
void aps_trustLinkJoinRet(app_trustLinkJoinStateRet_t cb);

u8 get_device_zdTcAware(void);
void set_device_zdTcAware(u8 aware);
#endif
