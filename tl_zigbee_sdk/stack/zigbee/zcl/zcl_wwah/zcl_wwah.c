...
_CODE_ZCL_ status_t zcl_wwah_useTrustCenterForClusterCmd(u8 srcEp, epInfo_t *pDstEpInfo, u8 disableDefaultRsp, u8 seqNo, clustersToExempt_t *pCmd)
{
    u8 status = ZCL_STA_SUCCESS;
    u8 len = 1 + pCmd->numOfClustersToExempt * sizeof(u16);
...
_CODE_ZCL_ status_t zcl_wwah_apsAckRequirementQueryRspCmd(u8 srcEp, epInfo_t *pDstEpInfo, u16 manuCode, u8 disableDefaultRsp, u8 seqNo, wwah_apsAcksRequireInfo_t *pInfo)
{
    u8 status = ZCL_STA_SUCCESS;
    u8 len = 1 + pInfo->num * sizeof(u16);
    ...
    *pBuf++ = pInfo->num;
    for (u8 i = 0; i < pInfo->num; i++) {
        *pBuf++ = LO_UINT16(pInfo->clusterID[i]);
        *pBuf++ = HI_UINT16(pInfo->clusterID[i]);
    }
...
_CODE_ZCL_ status_t zcl_wwah_trustCenterForClusterServerQueryRspCmd(u8 srcEp, epInfo_t *pDstEpInfo, u16 manufCode, u8 disableDefaultRsp, u8 seqNo, wwah_useTCForClusterInfo_t *pInfo)
{
    u8 status = ZCL_STA_SUCCESS;
    u8 len = 1 + pInfo->num * sizeof(u16);
...
_CODE_ZCL_ status_t zcl_wwah_surveyBeaconsRspCmd(u8 srcEp, epInfo_t *pDstEpInfo, u16 manufCode, u8 disableDefaultRsp, u8 seqNo, zcl_wwah_surveyBeaconsRspCmd_t *pCmd)
{
    u8 status = ZCL_STA_SUCCESS;
...
_CODE_ZCL_ static u8 zcl_wwah_clustersToExemptPrc(zclIncoming_t *pInMsg)
{
    ...
    if ((pInMsg->hdr.cmd == ZCL_CMD_WWAH_ENABLE_APS_LINK_KEY_AUTH) ||
        (pInMsg->hdr.cmd == ZCL_CMD_WWAH_DISABLE_APS_LINK_KEY_AUTH)) {
        g_zcl_apsLinkKeyAuthInfo.status = (pInMsg->hdr.cmd == ZCL_CMD_WWAH_ENABLE_APS_LINK_KEY_AUTH)
                                        ? APS_LINK_KEY_AUTH_ENABLE : APS_LINK_KEY_AUTH_DISABLE;
        g_zcl_apsLinkKeyAuthInfo.num = *pData++;
        g_zcl_apsLinkKeyAuthInfo.num = (g_zcl_apsLinkKeyAuthInfo.num > CLUSTER_ID_MAX_NUM)
                                     ? CLUSTER_ID_MAX_NUM : g_zcl_apsLinkKeyAuthInfo.num;
        for (u8 i = 0; i < g_zcl_apsLinkKeyAuthInfo.num; i++) {
        ...
        g_zcl_apsAcksRequireInfo.num = (g_zcl_apsAcksRequireInfo.num > CLUSTER_ID_MAX_NUM)
                                     ? CLUSTER_ID_MAX_NUM : g_zcl_apsAcksRequireInfo.num;
...
_CODE_ZCL_ static u8 zcl_wwah_apsAckRequirementQueryPrc(zclIncoming_t *pInMsg)
{
    ...
    if (UNICAST_MSG(pApsdeInd)) {
    	wwah_apsAcksRequireInfo_t *pInfo = &g_zcl_apsAcksRequireInfo;
        ...
        status = zcl_wwah_apsAckRequirementQueryRspCmd(pApsdeInd->indInfo.dst_ep, &dstEp, pInMsg->hdr.manufCode, TRUE, pInMsg->hdr.seqNum, pInfo);
...
_CODE_ZCL_ static u8 zcl_wwah_trustCenterForClusterServerQueryPrc(zclIncoming_t *pInMsg)
{
    ...
    if (UNICAST_MSG(pApsdeInd)) {
        wwah_useTCForClusterInfo_t *pInfo = &g_zcl_useTCForClusterInfo;
        ...
        status = zcl_wwah_trustCenterForClusterServerQueryRspCmd(pApsdeInd->indInfo.dst_ep, &dstEp, pInMsg->hdr.manufCode, TRUE, pInMsg->hdr.seqNum, pInfo);
...
_CODE_ZCL_ static u8 zcl_wwah_apsAckRequirementQueryRspPrc(zclIncoming_t *pInMsg)
{
    ...
    if (pInMsg->clusterAppCb) {
        zcl_wwah_apsAckRequirementQueryRspCmd_t cmd;
        TL_SETSTRUCTCONTENT(cmd, 0);

        cmd.numOfClusters = *pData++;
        cmd.pClusterID = cmd.pClusterID ? pData : NULL;
...
_CODE_ZCL_ static u8 zcl_wwah_trustCenterForClusterServerQueryRspPrc(zclIncoming_t *pInMsg)
{
    ...
    if (pInMsg->clusterAppCb) {
        ...
        cmd.numOfClusters = *pData++;
        cmd.pClusterID = cmd.numOfClusters ? pData : NULL;
