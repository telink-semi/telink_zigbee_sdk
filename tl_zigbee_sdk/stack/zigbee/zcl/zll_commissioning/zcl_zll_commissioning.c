...
bool initiatorProfileInterop = 0;
bool initiatorRxOnWhenIdle = 0;
u8 initiatorDevType = 0;
...
_CODE_ZCL_  static void zcl_zllGetGroupIdentifiersRequestHandler(u8 startIdx, epInfo_t *dstEpInfo)
{
    ...
    if (pResp) {
        ...
        zcl_sendCmd(g_zllTouchLink.devInfo.epId, dstEpInfo, ZCL_CLUSTER_TOUCHLINK_COMMISSIONING,
                    ZCL_CMD_ZLL_COMMISSIONING_GET_GROUP_IDENTIFIERS_RSP, TRUE,
...
_CODE_ZCL_ static void zcl_zllGetEndpointListRequestHandler(u8 startIdx, epInfo_t *dstEpInfo)
{
    ...
    if (pResp) {
        ...
        zcl_sendCmd(g_zllTouchLink.devInfo.epId, dstEpInfo, ZCL_CLUSTER_TOUCHLINK_COMMISSIONING,
                    ZCL_CMD_ZLL_COMMISSIONING_GET_ENDPOINT_LIST_RSP, TRUE,
...
_CODE_ZCL_ void	zcl_zllTouchLinkFinish(u8 status)
{
    ...
    //if (status == ZCL_ZLL_TOUCH_LINK_STA_SUCC || status == ZCL_ZLL_TOUCH_LINK_STA_EXIST) {
        zb_setPollRate(g_zllTouchLink.pollRateBackUp);
    //}
#endif

    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TL Finish: sta = %x, rx = %d, pwr = %d, chn = %d, pollRate = %d\n",
          status, MAC_IB()->rxOnWhenIdle, g_zb_txPowerSet, g_zllTouchLink.workingChannelBackUp, g_zllTouchLink.pollRateBackUp);
...
_CODE_ZCL_ static u8 zcl_touchLinkClientCmdHandler(zclIncoming_t *pInMsg)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] clientCmdRcv: cmd = 0x%x, lqi = %d\n", cmd, pApsdeInd->indInfo.lqi);
    ...
    case ZCL_CMD_ZLL_COMMISSIONING_SCAN:
        {
            ...
            if ((scanReq.transId == g_zllTouchLink.transId) ||
               (g_zllTouchLink.state & ZCL_ZLL_COMMISSION_STATE_TOUCHLINK_STARTORJOIN) == ZCL_ZLL_COMMISSION_STATE_TOUCHLINK_STARTORJOIN) {
                break;
            } else {
                if ((g_zllTouchLink.state == ZCL_ZLL_COMMISSION_STATE_TOUCHLINK_DISCOVERY) &&
                    scanReq.zllInfo.bf.factoryNew) {
                    break;
                }
            ...
            initiatorProfileInterop = scanReq.zllInfo.bf.profileInterop;
            initiatorRxOnWhenIdle = scanReq.zbInfo.bf.rxOnWihleIdle;
            initiatorDevType = scanReq.zbInfo.bf.logicDevType;

            DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] RX scan: transID = 0x%x\n", scanReq.transId);
    ...
    case ZCL_CMD_ZLL_COMMISSIONING_NETWORK_START:
        {
            ...
            if (g_zllTouchLink.networkStartInfo) {
                ...
                if (pReq->transId == g_zllTouchLink.transId) {
                    ...
                    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] RX nwkSstart: chn = %d, addr = 0x%x\n", pReq->logicalChannel, pReq->nwkAddr);
...
#endif
        {
            ...
            if (g_zllTouchLink.networkStartInfo) {
                ...
                if (pReq->transId == g_zllTouchLink.transId) {
                    ...
                    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] RX nwkJoin: chn = %d, pan = 0x%x, addr = 0x%x\n", pReq->logicalChannel, pReq->panId, pReq->nwkAddr);
...
_CODE_ZCL_ static u8 zcl_zllCommissionServerCmdHandler(zclIncoming_t *pInMsg)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] serverCmdRcv: cmd = %x, lqi = %x, sta = %x\n", cmd, pApsdeInd->indInfo.lqi, g_zllTouchLink.state);
    ...
    case ZCL_CMD_ZLL_COMMISSIONING_SCAN_RSP:
        {
            ...
            if ((g_zllTouchLink.state != ZCL_ZLL_COMMISSION_STATE_TOUCHLINK_DISCOVERY) ||
                (!g_zllTouchLink.disc) || (resp.transId != g_zllTouchLink.transId)) {
                status =  ZCL_STA_INVALID_VALUE;
                break;
            }

            DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] RX scanRsp: transID = 0x%x, chn = %d\n", resp.transId, resp.logicalChannel);
    ...
    case ZCL_CMD_ZLL_COMMISSIONING_NETWORK_START_RSP:
        {
            ...
            if (nscr) {
                ...
                if (nscr->transId == g_zllTouchLink.transId) {
                    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] RX startRsp: sta = 0x%x, chn = %d, pan = 0x%x\n", nscr->status, nscr->logicalChannel, nscr->panId);

                    zcl_zllTouchLinkStartNetworkStartOrJoinTimerStop();
                    TL_ZB_TIMER_SCHEDULE(zcl_zllTouchLinkNetworkStartResponseHandler, nscr, ZB_MIN_STARTUP_DELAY_TIME);
                } else {
                    ev_buf_free((u8 *)nscr);
                }
            }
    ...
    case ZCL_CMD_ZLL_COMMISSIONING_NETWORK_JOIN_END_DEVICE_RSP:
        {
            ...
            DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] RX joinRsp: sta = 0x%x\n", resp.status);
...
_CODE_ZCL_ void zcl_touchLinkStart(void)
{
    if ((g_zllTouchLink.state != ZCL_ZLL_COMMISSION_STATE_IDLE) ||
        (TL_ZB_MAC_STATUS_GET() != ZB_MAC_STATE_NORMAL)) {
        if (g_zllCommission.appCb->touchLinkCallback) {
    ...
    g_zllTouchLink.pollRateBackUp = zb_getPollRate();

    DEBUG(DEBUG_TOUCHLINK_EN,
          "[ZLL] touchLink start: transId = 0x%x, isNew = %x, pwr = %d, bkupChn = %d, bkupPoll = %d\n",
          g_zllTouchLink.transId, g_zllTouchLink.zllInfo.bf.factoryNew, g_zllTouchLink.commissionTxPower,
          g_zllTouchLink.workingChannelBackUp, g_zllTouchLink.pollRateBackUp);
...
_CODE_ZCL_ void zcl_touchLinkReset2FN(void)
{
    if (!zdo_ifZdoNwkManagerIdle()) {
        return;
    }

    g_zllTouchLink.rst2FactoryNew = 1;
}
