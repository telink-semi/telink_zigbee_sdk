...
_CODE_ZCL_ void tl_zbNwkZllCommissionScanConfirm(void)
{
    ...
    g_zllTouchLink.workingChannelBackUp = channel; //store the select channel
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] scanCnf: curChn = %d, selectChn = %d\n", MAC_IB()->phyChannelCur, channel);
    ...
    if (g_bdbCtx.role == BDB_COMMISSIONING_ROLE_INITIATOR) {
        ...
        ZB_PIB_SHORT_ADDRESS() = nwkAddr;

        /* network key generate */
        zb_routerStart();
    } else {
        /* use the TL channel to send response */
        ...
        startParams->logicalChannel = channel;
...
_CODE_ZCL_ void zcl_zllTouchLinkNetworkStartDirectJoin(void *arg)
{
    if (g_zllTouchLink.transId) {
        ...
        if ((g_zllTouchLink.networkStartInfo->joinNetworkMode == ZCL_ZLL_COMMISSIONING_TOUCH_LICK_MODE_START) &&
            ...
            req.capabilityInfo.devType = (initiatorDevType & 0x02) ? 0 : 1;
            req.capabilityInfo.rcvOnWhenIdle = initiatorRxOnWhenIdle;

            DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] directJoin: addr = 0x%x, rx = %x, devType = %x\n",
                  req.nwkAddr, req.capabilityInfo.rcvOnWhenIdle, req.capabilityInfo.devType);
...
_CODE_ZCL_ static void zcl_zllTouchLinkStartRouteRequest(void)
{
    ...
    ZB_PIB_SHORT_ADDRESS() = startParams->nwkAddr;
...
_CODE_ZCL_ static void zcl_zllTouchLinkNetworkJoinRealjoin(void)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] realJoin: chnBackup = %d\n", g_zllTouchLink.workingChannelBackUp);
    ...
    NWK_NIB().panId = pReq->panId;

    /* Update network address */
    NIB_NETWORK_ADDRESS() = pReq->nwkAddr;
    ZB_PIB_SHORT_ADDRESS() = pReq->nwkAddr;
    ...
    if (af_nodeDevTypeGet() == DEVICE_TYPE_END_DEVICE) {
        ...
        zb_rejoinReq(1 << pReq->logicalChannel, zdo_cfg_attributes.config_nwk_scan_duration);
...
_CODE_ZCL_ static s32 zcl_zllTouchLinkNetworkStartOrJoinNwkLeave(void *arg)
{
    ...
    if (!is_device_factory_new()) {
        DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] leave...\n");
        ...
        DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] joinMode: %x\n", g_zllTouchLink.networkStartInfo->joinNetworkMode);
...
_CODE_ZCL_ s32 zcl_zllTouchLinkNetworkStartResponseHandler(void *arg)
{
    ...
    if (p->status == SUCCESS) {
        ...
        memcpy(NWK_NIB().extPANId, p->epanId, 8);
        MAC_IB()->panId = p->panId;
        MAC_IB()->phyChannelCur = p->logicalChannel;

        g_zllTouchLink.workingChannelBackUp = p->logicalChannel;

        DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] rejoin: chn = %d, panId = 0x%x\n", p->logicalChannel, p->panId);

        //wait for target's network is formed
        TL_ZB_TIMER_SCHEDULE(zcl_zllRejoinStart, (void *)((u32)p->logicalChannel), 200);
    } else {
...
_CODE_ZCL_ s32 zcl_zllTouchLinkNetwokJoinResponseHandler(void *arg)
{
    if ((g_zllTouchLink.zbInfo.bf.logicDevType == DEVICE_TYPE_END_DEVICE) &&
        ...
        u8 chn = g_zllTouchLink.workingChannelBackUp;
        zb_rejoinSecModeSet(REJOIN_SECURITY);
        zb_rejoinReq(1 << chn, g_bdbAttrs.scanDuration);
    } else {
        zcl_zllTouchLinkFinish(ZCL_ZLL_TOUCH_LINK_STA_SUCC);
    }
    return -1;
...
_CODE_ZCL_ void zcl_zllTouchLinkNetworkStartOrJoin(void *arg)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] startOrJoin: curChn = %d, onNwk = %d\n",
          MAC_IB()->phyChannelCur, BDB_ATTR().nodeIsOnANetwork);
    ...
    if (!BDB_ATTR().nodeIsOnANetwork) {
        ...
        if (g_zllTouchLink.zbInfo.bf.logicDevType == DEVICE_TYPE_ROUTER) {
            ...
            touchlink_discovery_network(ZB_ZLL_PRIMARY_CHANNELS_MASK); //initiator
        ...
        if (!selfForm) {
            ...
            DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TX nwkStart: chn = %d, pan = 0x%x, addr = 0x%x\n",
                  startReq.logicalChannel, startReq.panId, startReq.nwkAddr);
        ...
        DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TX nwkJoin: cmd = 0x%x, chn = %d, pan = 0x%x, addr = 0x%x\n",
               cmd, joinReq.logicalChannel, joinReq.panId, joinReq.nwkAddr);
...
u8 g_zllRejoinAttempts = 0;
void zcl_zllRejoinAgain(void)
{
    u8 chn = g_zllTouchLink.workingChannelBackUp;
    //wait for target's network is formed
    TL_ZB_TIMER_SCHEDULE(zcl_zllRejoinStart, (void *)((u32)chn), 1000);
}

void zcl_touchLinkDevStartIndicate(u8 status)
{
    ...
    if (status == SUCCESS) {
        ...
        if (g_zllTouchLink.zbInfo.bf.logicDevType == DEVICE_TYPE_END_DEVICE) {
            ...
            DEBUG(DEBUG_TOUCHLINK_EN, "zllRejoinAttempts = %d\n", g_zllRejoinAttempts);

            if (g_zllRejoinAttempts < 3) {
                zcl_zllRejoinAgain();
                return;
            } else {
                g_zllRejoinAttempts = 0;
            }
        }
...
_CODE_ZCL_ void zcl_zllTouchLinkResetFactoryReq(void *arg)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TX FN-RST: curChn = %d\n", g_zbMacPib.phyChannelCur);
