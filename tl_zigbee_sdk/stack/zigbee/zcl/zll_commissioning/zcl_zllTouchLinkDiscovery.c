...
_CODE_ZCL_ s32 zcl_touchLinkIdentifyRequestDone(void *arg)
{
    u32 sta = (u32)arg;
...
_CODE_ZCL_ static void zcl_zllTouchLinkIdentifyRequest(void *arg)
{
    ...
    req.identifyDuration = 10;

    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TX idf: duration = %d, onNWK = %d\n", req.identifyDuration, BDB_ATTR().nodeIsOnANetwork);
    ...
    if (BDB_ATTR().nodeIsOnANetwork) {
        if (ZB_EXTPANID_CMP(peerInfo->epanId, NWK_NIB().extPANId)) {
            DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] nwkUpdateId: rcvID = 0x%x, curID = 0x%x, rcvChn = %d, curChn = %d\n",
                  peerInfo->nwkUpdateId, NWK_NIB().updateId, peerInfo->logicalChannel, g_zllTouchLink.workingChannelBackUp);
            ...
            if (peerInfo->nwkUpdateId != NWK_NIB().updateId) {
                ...
                networkUpdateCmd.nwkUpdateId = (peerInfo->nwkUpdateId > NWK_NIB().updateId) ? peerInfo->nwkUpdateId : NWK_NIB().updateId;
                networkUpdateCmd.logicalChannel = g_zllTouchLink.workingChannelBackUp;
                networkUpdateCmd.panId = NWK_NIB().panId;
                networkUpdateCmd.nwkAddr = NWK_NIB().nwkAddr;

                DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TX nwkUpdateId: updateId = %x, chn = %d\n", networkUpdateCmd.nwkUpdateId, networkUpdateCmd.logicalChannel);
            ...
            if (peerInfo->nwkUpdateId > NWK_NIB().updateId) {
                ...
                MAC_IB()->phyChannelCur = peerInfo->logicalChannel;

                if (af_nodeDevTypeGet() == DEVICE_TYPE_END_DEVICE) {
                    zb_rejoinSecModeSet(REJOIN_SECURITY);
                    zb_rejoinReq(1 << peerInfo->logicalChannel, zdo_cfg_attributes.config_nwk_scan_duration);
                    return;
                } else {
                    //set and store channel
    ...
    /* Is initiator address assignment capable? Set bdbCommissioningStatus to NOT_AA_CAPABLE*/
...
_CODE_ZCL_ void zcl_zllTouchLinkDeviceInformationRequest(void *arg)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] TX devInfoReq: idx = %d\n", req.startIdx);
...
_CODE_ZCL_  void zcl_zllTouchLinkScanRequestHandler(epInfo_t *srcEp, u8 seqNo)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] scanRspSend: transId = %x, chn = %d\n", resp.transId, resp.logicalChannel);
...
_CODE_ZCL_ static void zcl_zllTouchLinkDiscoverydone(void *arg)
{
    ...
    DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] discDone: num = %d\n", pDisc->targetNum);

    if (pDisc->targetNum) {
        zll_touchLinkScanInfo *pScanInfo = NULL;
        g_zllTouchLink.opIdx = 0;

        u8 i = 0;
        for (i = 0; i < pDisc->targetNum; i++) {
            pScanInfo = &pDisc->scanList[i];
        ...
        if ((i >= pDisc->targetNum) && (pDisc->targetNum > 1)) {
            u8 lqiMax = 0;
            for (u8 n = 0; n < pDisc->targetNum; n++) {
                pScanInfo = &pDisc->scanList[n];
        ...
        DEBUG(DEBUG_TOUCHLINK_EN, "[ZLL] discDone: idx = %d\n", g_zllTouchLink.opIdx);

        pScanInfo = &pDisc->scanList[g_zllTouchLink.opIdx];

        /* store and switch the channel to the target node's */
        ...
        DEBUG(DEBUG_TOUCHLINK_EN, "tl_rstFN = %d, num = %d\n", g_zllTouchLink.rst2FactoryNew, pScanInfo->numOfSubdevices);

        if (g_zllTouchLink.rst2FactoryNew) {
            g_zllTouchLink.rst2FactoryNew = 0;
