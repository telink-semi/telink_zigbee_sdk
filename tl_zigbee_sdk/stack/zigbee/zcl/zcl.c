...
#ifdef ZCL_POLL_CTRL
    //for test-case: POLL-TC-02S
    if ((clusterId == ZCL_CLUSTER_GEN_POLL_CONTROL) &&
        (pWriteRec->attrID == ZCL_ATTRID_CHK_IN_INTERVAL)) {
        u32 checkInInterval = BUILD_U32(pWriteRec->attrData[0], pWriteRec->attrData[1], pWriteRec->attrData[2], pWriteRec->attrData[3]);
        u16 attrLen = 0;
        u32 longPollInterval = 0x04;

        zcl_getAttrVal(endpoint, ZCL_CLUSTER_GEN_POLL_CONTROL, ZCL_ATTRID_LONG_POLL_INTERVAL,
                       &attrLen, (u8 *)&longPollInterval);

        if (checkInInterval < longPollInterval) {
...
_CODE_ZCL_ void zcl_cmdHandler(void *pCmd)
{
    ...
    g_sysDiags.lastMessageLQI = inMsg.msg->indInfo.lqi;
    /* diagnostics last message RSSI */
    g_sysDiags.lastMessageRSSI = inMsg.msg->indInfo.rssi;
