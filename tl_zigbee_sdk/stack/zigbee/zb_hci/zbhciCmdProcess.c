...
extern void zbhci_zclZddHandle(void *arg);
...
static void zbhciNodeDescRspMsgPush(void *arg)
{
    ...
    ZB_LEBESWAP(((u8 *)&(rsp->node_descriptor.manufacturer_code)), 2);
...
static void match_desc_req_handler(void *arg)
{
    ...
    if (req) {
        ...
        ptr += 2;
        req->num_in_clusters = inNum;
        req->num_out_clusters = outNum;
        ptr += 2;
...
static void zbhci_bdbCmdHandler(void *arg)
{
    ...
    if (cmdID == ZBHCI_CMD_BDB_COMMISSION_FORMATION) {
        ...
        u32 u32Value = (1 << p[0]);
        g_bdbAttrs.primaryChannelSet = (u32Value & BDBC_TL_PRIMARY_CHANNEL_SET);
        g_bdbAttrs.secondaryChannelSet = (u32Value & BDBC_TL_SECONDARY_CHANNEL_SET);		
...
#if ZB_COORDINATOR_ROLE
    else if (cmdID == ZBHCI_CMD_SET_BACKWARDS_COMPTIBILITY_MODE) {
        if (p[0] == 0) {
            g_ss_tcPolicy.reqICorPrePassphrase = 2;
        } else {
            g_ss_tcPolicy.reqICorPrePassphrase = 3;
        }
    }
#endif
    ...
    if (cmdID == ZBHCI_CMD_NODES_JOINED_GET_REQ) {
        ...
        if (rsp) {
            ...
            for (u8 i = 0; i < validcnt; i++) {
                ...
                if (tl_zbShortAddrByExtAddr(&shortAddr, (u8 *)pMacAddr, &idx) == RET_INVALID) {
                    shortAddr = 0xfffe;
                }
        ...
        *pBuf++ = MAC_IB()->phyChannelCur;
        ...
        if (rsp) {
            ...
            for (s32 m = 0; m < rsp->info.childNodesNum; m++) {
                ...
                ZB_SHORT_ADDR_REVERT((u8 *)&tNwkAddr, (u8 *)&rsp->list[m].nwkAddr);
...
void uart_send_ota_block_request(void)
{
    u8 array[8] = {0};
    u8 *pBuf = array;
    COPY_U32TOBUFFER_BE(pBuf, ota_info.otaFileOffset);
...
void zbhciCmdHandler(u16 msgType, u16 msgLen, u8 *p)
{
    ...
    if (cmdInfo) {
        ...
        case ZBHCI_CMD_ZCL_ZDD_SET_ANJ_TIME:
            TL_SCHEDULE_TASK(zbhci_zclZddHandle, cmdInfo);
            break;
#endif
