...
typedef enum {
    ...
    ZBHCI_CMD_SET_BACKWARDS_COMPTIBILITY_MODE,
    ...
    ZBHCI_CMD_ZCL_ZDD_SET_STATE                 = 0x0220,
    ZBHCI_CMD_ZCL_ZDD_SET_STATE_RESPONSE        = 0x8220,
    ZBHCI_CMD_ZCL_ZDD_SET_ANJ_TIME              = 0x0221,
} teHCI_MsgType;
...
#define ZB_LEBESWAPU16(value)                   do { \
                                                    (value = (value >> 8) | (value << 8)); \
                                                } while(0)

#define ZB_LEBESWAP(ptr, len)                   do { \
...
#define ZB_IEEE_ADDR_REVERT(tar, addr)          do { \
                                                    for (int i = 0; i < EXT_ADDR_LEN; i++) { \
                                                        (tar)[i] = (addr)[EXT_ADDR_LEN - i - 1]; \
                                                    } \
                                                } while(0)

#define ZB_SHORT_ADDR_REVERT(tar, addr)         do { \
                                                    for (int i = 0; i < 2; i++) { \
                                                        (tar)[i] = (addr)[2 - i - 1]; \
                                                    } \
                                                } while(0)

#define ZB_LINK_KEY_REVERT(tar, addr)           do { \
...
/**
 * @brief  the hci response command carry in the MAC address of the nodes which have joined the network
 */
...
typedef struct _attribute_packed_ {
    ...
    zbhci_mgmt_nodesJoined_info_t addrList[5];  /* the mac address list */
...
#if ZB_TEST_ENABLE
void zbhciProcessIncomingSerialCmd(u16 msgType, u16 msgLen, u8 *p);
#else
