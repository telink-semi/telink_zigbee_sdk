...
_attribute_data_no_init_ tl_zbtaskq_user_t taskQ_user;
...
/* timeout waiting for transport nwk key during association join or security rejoin, 5 seconds */
u32 TRANSPORT_NETWORK_KEY_WAIT_TIME = (5 * 1000);
...
/* whether ZDD send route response on be half of ZVD */
bool ZB_TRUST_ROUTE_BEHALF = TRUE;
...
_attribute_data_no_init_ tx_data_queue g_txQueue[TX_QUEUE_BN];
...
//APS well-known PSK
/* ZigBeeAlliance18 */
const u8 apsWellKnownPSK[] = {
    0x5a, 0x69, 0x67, 0x42, 0x65, 0x65, 0x41, 0x6c,
    0x6c, 0x69, 0x61, 0x6e, 0x63, 0x65, 0x31, 0x38
};
...
#if defined(ZB_NWK_DISTRIBUTED_ADDRESS_ASSIGN) && (ZB_NWK_DISTRIBUTED_ADDRESS_ASSIGN == 1)
    .addrAlloc                          = NWK_ADDRESS_ALLOC_METHOD_DISTRIBUTED,
#if (ZB_COORDINATOR_ROLE || ZB_ROUTER_ROLE)
    .maxRouters                         = NWK_MAX_ROUTERS,
#endif
#elif defined(ZB_NWK_STOCHASTIC_ADDRESS_ASSIGN) && (ZB_NWK_STOCHASTIC_ADDRESS_ASSIGN == 1)
