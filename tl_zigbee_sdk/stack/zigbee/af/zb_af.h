...
//max nsdu length = aMaxPHYFrameSize - (nwkcMACFrameOverhead + nwkcMinHeaderOverhead) (D.4 aMaxMACFrameSize)
#define	AF_NSDU_MAX_LEN                         (ZB_PHY_MAX_PACKET_SIZE - (ZB_MAC_FRAME_HEADER + ZB_NWK_FRAME_HEADER))
#define AF_ASDU_MAX_LEN                         (MAX_BUFFER_SIZE - (ZB_MAC_FRAME_HEADER + ZB_NWK_FRAME_HEADER + ZB_APS_FRAME_HEADER) \
                                                                 - OFFSETOF(apsdeDataInd_t, asdu))
...
enum {
    ...
    MAC_CAP_ALLOCATE_ADDRESS                    = 0x80 //always set for rev23
};
...
enum {
    ...
    ND_SERVER_MASK_REV20                        = (ND_STACK_RVE_20 << 9),
    ND_SERVER_MASK_REV21                        = (ND_STACK_RVE_21 << 9),
    ND_SERVER_MASK_REV22                        = (ND_STACK_RVE_22 << 9),
    ND_SERVER_MASK_REV23                        = (ND_STACK_RVE_23 << 9)
};
...
typedef struct _attribute_packed_ {
    ...
    u8 fragmentation_supported:1;//R23
     ...
     */
    u16 manufacturer_code;//Manufacturer code
    /*
     ...
     * system.0: Extended Active Endpoint list Available; 1: Extended Simple Descriptor list Available;
     * 2~27: reserved
     */
    u8 desc_capability_field;//Descriptor capability field
} node_descriptor_t;//13
 ...
 *
 * @param  mode - the power mode will be used
 *
 * @return none
 */
 ...
 *
 * @param  manuCode - the Manufacturer code will be used
 *
 * @return none
 */
 ...
 *
 * @param  revision - the stack revision will be used
 *
 * @return none
 */
...
/**
 * @brief  Copy simple descriptor interface used building simple descriptor response message
 *
 * @param  dst - destination address to store simple descriptor info
 * @param  sd  - local simple descriptor
 *
 ...
 *
 * @param  dst - destination address to hold node descriptor info
 *
 * @return none
 */
...
/**
 * @brief  Interface to get the endpoint descriptor
 *
 * @param  none
 *
 * @return the point to the endpoint descriptor
 */
...
/**
 * @brief  Check whether simple descriptor request's profile identifier
 *         matched with local registered end point info
 *
 ...
 * @param  cb          - callback function to handle the APS data sent to this endpoint
 * @param  cnfCb       - data send confirm callback function
 *
 * @return TRUE or FALSE
 */
...
/**
 * @brief  Unregister an endpoint and it's simple descriptor
 *
 * @param  ep - endpoint
 *
 * @return TRUE or FALSE
 ...
 * @param  pDstEpInfo - destination information
 * @param  clusterId  - cluster identifier
 * @param  cmdPldLen  - data length
 * @param  cmdPld     - data payload
 * @param  apsCnt     - the APS count
 *
