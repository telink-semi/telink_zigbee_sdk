...
#define ND_DESC_STACK_REV               ND_SERVER_MASK_REV23
#else
#define ND_DESC_STACK_REV               ND_SERVER_MASK_REV22
#endif

#if ZB_COORDINATOR_ROLE
    #define LOGICAL_TYPE                DEVICE_TYPE_COORDINATOR
    #define SERVER_MASK                 (ND_DESC_STACK_REV | \
                                         ND_SERVER_MASK_PRIMARY_TRUST_CENTER | \
                                         ND_SERVER_MASK_NETWORK_MANAGER)
    #define MAC_CAPABILITY_FLAGS        (MAC_CAP_DEVICE_TYPE_FFD | \
    ...
    #define MAC_CAPABILITY_FLAGS        (MAC_CAP_DEVICE_TYPE_RFD | \
                                         MAC_CAP_RX_ON_WHEN_IDLE | \
                                         MAC_CAP_ALLOCATE_ADDRESS)
    #else
    #define MAC_CAPABILITY_FLAGS        (MAC_CAP_DEVICE_TYPE_RFD | \
...
{
    ndt.manufacturer_code = manuCode;
}
...
/*********************************************************************
 * @brief  Copy simple descriptor interface used building
 *         simple descriptor response message
 *
...
_CODE_AF_ u8 af_simpleDescriptorCopy(u8 *dst, af_simple_descriptor_t *sd)
{
    ...
    *ptr++ = sd->endpoint;
    *ptr++ = LO_UINT16(sd->app_profile_id);
    *ptr++ = HI_UINT16(sd->app_profile_id);
    *ptr++ = LO_UINT16(sd->app_dev_id);
    *ptr++ = HI_UINT16(sd->app_dev_id);
...
/*********************************************************************
 * @brief  AF layer endpoint descriptor register interface,
 *         call this function to add ep descriptor to the AF layer
 *
