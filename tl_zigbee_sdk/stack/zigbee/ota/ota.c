...
#define OTA_BACKUP_DATA_SIZE            128
...
ota_data_backup_t g_ota_backup_data;
//server side
...
void ota_queryStop(void)
{
    if (!g_otaCtx.isOtaServer) {
        if (ev_timer_exist(&otaTimer)) {
            if (otaTimer.cb == ota_periodicQueryServerCb) {
                ev_unon_timer(&otaTimer);
            }
        }
    }
    ...
    for (u8 i = 0; i < len; i++) {
        ...
        case OTA_FLAG_IMAGE_ELEMENT:
            ...
            if ((otaClientInfo.otaElementTag == OTA_UPGRADE_IMAGE_TAG_ID) ||
                ...
                if (otaClientInfo.otaElementLen - (otaClientInfo.otaElementPos + g_ota_backup_data.len) <= dataSize) {
                    //the last packet
                    copyLen = otaClientInfo.otaElementLen - (otaClientInfo.otaElementPos  + g_ota_backup_data.len);
                    pldLen = copyLen;
                    last_pkt = TRUE;
                }

                if (g_ota_backup_data.len) {
                    memcpy(g_ota_backup_data.des, g_ota_backup_data.pld, g_ota_backup_data.len);
                    des_oft += g_ota_backup_data.len;
                }

                recvd_len = pldLen + g_ota_backup_data.len;
                if (recvd_len > OTA_BACKUP_DATA_SIZE) {
                	return ZCL_STA_INVALID_IMAGE;
                }

                if (last_pkt) {
                    // the last packet
                    g_ota_backup_data.len = 0;
                    memcpy(&g_ota_backup_data.des[des_oft], &pData[i], copyLen);
                    copyLen = recvd_len;
                } else {
                    g_ota_backup_data.len = (recvd_len & 0x0f);
                    //copy the left data to backup buffer
                    memcpy(g_ota_backup_data.pld, &pData[i + pldLen - g_ota_backup_data.len], g_ota_backup_data.len);

                    copyLen = recvd_len - g_ota_backup_data.len;
                    memcpy(&g_ota_backup_data.des[des_oft], &pData[i], copyLen - des_oft);
                }
                ...
                if (otaClientInfo.otaElementTag == OTA_UPGRADE_IMAGE_AES_TAG_ID) {
                    ...
                    for (u8 j = 0; j < copyLen; j += 16) {
                        ...
                        memcpy(tmpBuf, &g_ota_backup_data.des[j], 16);
                        drv_aes_decrypt((u8 *)otaAesKey, tmpBuf, &g_ota_backup_data.des[j]);
                    }
                }
                ...
                if (dataEndPos - otaClientInfo.otaElementPos <= 4) {
                    crcLen = 0;
                } else if (dataEndPos - 4 - otaClientInfo.otaElementPos >= copyLen) {
                    crcLen = copyLen;  //dataSize;
                } else {
                    crcLen = dataEndPos - 4 - otaClientInfo.otaElementPos;
                }
                otaClientInfo.crcValue = xcrc32(&g_ota_backup_data.des[0], crcLen, otaClientInfo.crcValue);

                //write image to flash
                if ((otaClientInfo.otaElementPos < FLASH_TLNK_FLAG_OFFSET + 1) &&
                   ((otaClientInfo.otaElementPos + copyLen) >= FLASH_TLNK_FLAG_OFFSET + 1)) {
                    if (g_ota_backup_data.des[(FLASH_TLNK_FLAG_OFFSET - otaClientInfo.otaElementPos)] != TL_IMAGE_START_FLAG) {
                        return ZCL_STA_INVALID_IMAGE;
                    }
                    g_ota_backup_data.des[(FLASH_TLNK_FLAG_OFFSET - otaClientInfo.otaElementPos)] = 0xff;
                }
                u32 baseAddr = (mcuBootAddr) ? 0 : FLASH_ADDR_OF_OTA_IMAGE;
                if (flash_writeWithCheck(baseAddr + otaClientInfo.otaElementPos, copyLen, &g_ota_backup_data.des[0]) != TRUE) {
                    return ZCL_STA_INVALID_IMAGE;
                }

                otaClientInfo.otaElementPos += copyLen;
                zcl_attr_fileOffset += pldLen;//copyLen;
