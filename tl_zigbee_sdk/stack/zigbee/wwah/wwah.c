...
wwah_surveyBeaconReqInfo_t g_zcl_surveyBeaconReqInfo;
...
static void zcl_wwah_reqNewApsLinkKey(void *arg)
{
    ...
    zb_apsmeRequestKeyReq(&requestKey, NULL);
}
...
static void wwah_nwkDiscCnfCb(void)
{
    beaconSurvey_t beacon[6];
    zcl_wwah_surveyBeaconsRspCmd_t rsp;

    rsp.pBeacon = beacon;
    rsp.numOfBeacons = g_zb_neighborTbl.additionNeighborNum;

    for (u8 i = 0; i < rsp.numOfBeacons; i++) {
        beacon[i].deviceShort = g_zb_neighborTbl.additionNeighborTbl[i].shortAddr;
        beacon[i].rssi = g_zb_neighborTbl.additionNeighborTbl[i].rssi;
        beacon[i].mask.bits.tc_connectivity = g_zb_neighborTbl.additionNeighborTbl[i].tc_connectivity;
        beacon[i].mask.bits.long_uptime = g_zb_neighborTbl.additionNeighborTbl[i].long_uptime;
    }

    zcl_wwah_surveyBeaconsRspCmdSend(g_zcl_surveyBeaconReqInfo.srcEp, &g_zcl_surveyBeaconReqInfo.dstEpInfo,
                                     MANUFACTURER_CODE_AMAZON, TRUE, g_zcl_surveyBeaconReqInfo.seqNum, &rsp);
}

static status_t wwah_surveyBeaconsHandler(zclIncomingAddrInfo_t *pAddrInfo, zcl_wwah_surveyBeaconsCmd_t *pSurveyBeaconsCmd)
{
    status_t status = ZCL_STA_SUCCESS;

    if (pSurveyBeaconsCmd->standardBeacons) {
        u32 scanChannels = aps_ib.aps_channel_mask;
        u8 scanDuration = g_bdbAttrs.scanDuration;

        if (zb_nwkDiscovery(scanChannels, scanDuration, wwah_nwkDiscCnfCb)) {
            status = ZCL_STA_ACTION_DENIED;
        } else {
            g_zcl_surveyBeaconReqInfo.seqNum = pAddrInfo->seqNum;
            g_zcl_surveyBeaconReqInfo.srcEp = pAddrInfo->dstEp;
            g_zcl_surveyBeaconReqInfo.dstEpInfo.dstEp = pAddrInfo->srcEp;
            g_zcl_surveyBeaconReqInfo.dstEpInfo.profileId = pAddrInfo->profileId;
            g_zcl_surveyBeaconReqInfo.dstEpInfo.dstAddrMode = APS_SHORT_DSTADDR_WITHEP;
            g_zcl_surveyBeaconReqInfo.dstEpInfo.dstAddr.shortAddr = pAddrInfo->srcAddr;
            g_zcl_surveyBeaconReqInfo.dstEpInfo.txOptions |= APS_TX_OPT_ACK_TX;
            if (pAddrInfo->apsSec) {
                g_zcl_surveyBeaconReqInfo.dstEpInfo.txOptions |= APS_TX_OPT_SECURITY_ENABLED;
            }

            //wait for Active Scan done.
...
status_t zcl_wwahCb(zclIncomingAddrInfo_t *pAddrInfo, u8 cmdId, void *cmdPayload)
{
    ...
    if (pAddrInfo->dirCluster == ZCL_FRAME_CLIENT_SERVER_DIR) {
        ...
        case ZCL_CMD_WWAH_SURVEY_BEACONS:
            status = wwah_surveyBeaconsHandler(pAddrInfo, (zcl_wwah_surveyBeaconsCmd_t *)cmdPayload);
            break;
        case ZCL_CMD_WWAH_DISABLE_OTA_DOWNGRADES:
        case ZCL_CMD_WWAH_DISABLE_MGMT_LEAVE_WITHOUT_REJOIN:
        case ZCL_CMD_WWAH_DISABLE_TOUCHLINK_INTERPAN_MSG_SUPPORT:
