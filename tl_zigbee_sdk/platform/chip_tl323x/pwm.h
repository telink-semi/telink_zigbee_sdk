...
/**
 * @brief     This function servers to set pwm dead time.
 * @param[in] id   - variable of enum to select the pwm number,Value range 0~5.
 * @param[in] dead - variable of the dead time.
 * @return    none.
 * @note      Only add the dead band delay time(low level) before the rising edge of pulse, and the falling edge of pulse remains unchanged.
 */
static inline void pwm_set_dead(pwm_id_e id, unsigned short dead)
{
