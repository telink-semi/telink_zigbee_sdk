...
unsigned char flash_write_status_mid1571cd(unsigned short data, unsigned int mask)
{
    ...
    unsigned short status = flash_read_status_mid1571cd();
    if (data != (status & mask)) //To reduce the operation of the status register.
    {
        ...
        status = flash_read_status_mid1571cd();
    }
    if (data == (status & mask)) {
        return 1;
    }
...
{
    return flash_write_status_mid1571cd(data, FLASH_WRITE_STATUS_BP_MID1571CD);
}
...
{
    return flash_write_status_mid1571cd(FLASH_LOCK_NONE_MID1571CD, FLASH_WRITE_STATUS_BP_MID1571CD);
}
...
{
    return flash_read_status_mid1571cd() & FLASH_WRITE_STATUS_BP_MID1571CD;
}
...
{
    flash_write_status_mid1571cd(data, FLASH_WRITE_STATUS_OTP_MID1571CD);
}
