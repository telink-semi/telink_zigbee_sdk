...
unsigned char flash_write_status_mid1271cd(unsigned short data, unsigned int mask)
{
    ...
    unsigned short status = flash_read_status_mid1271cd();
    if (data != (status & mask)) //To reduce the operation of the status register.
    {
        ...
        status = flash_read_status_mid1271cd();
    }
    if (data == (status & mask)) {
        return 1;
    }
...
{
    return flash_write_status_mid1271cd(data, FLASH_WRITE_STATUS_BP_MID1271CD);
}
...
{
    return flash_write_status_mid1271cd(FLASH_LOCK_NONE_MID1271CD, FLASH_WRITE_STATUS_BP_MID1271CD);
}
...
{
    return flash_read_status_mid1271cd() & FLASH_WRITE_STATUS_BP_MID1271CD;
}
...
{
    flash_write_status_mid1271cd(data, FLASH_WRITE_STATUS_OTP_MID1271CD);
}
