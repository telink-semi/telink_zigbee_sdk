...
unsigned char flash_write_status_mid124585(unsigned short data, unsigned int mask)
{
    ...
    unsigned short status = flash_read_status_mid124585();
    if (data != (status & mask)) //To reduce the operation of the status register.
    {
        ...
        status = flash_read_status_mid124585();
    }
    if (data == (status & mask)) {
        return 1;
    }
...
unsigned char flash_write_config_mid124585(unsigned char data, unsigned char mask)
{
    ...
    unsigned char config = flash_read_config(0, FLASH_READ_CONFIGURE_CMD);
    if (data != (config & mask)) //To reduce the operation of the config register.
    {
        config = data | (config & ~(mask));
        flash_write_config(0, FLASH_WRITE_CONFIGURE_CMD_2, config);
...
{
    return flash_write_status_mid124585(data, FLASH_WRITE_STATUS_BP_MID124585);
}
...
{
    return flash_write_status_mid124585(FLASH_LOCK_NONE_MID124585, FLASH_WRITE_STATUS_BP_MID124585);
}
...
{
    return flash_read_status_mid124585() & FLASH_WRITE_STATUS_BP_MID124585;
}
...
{
    flash_write_config_mid124585(data, FLASH_WRITE_CONFIG_OTP_MID124585);
}
