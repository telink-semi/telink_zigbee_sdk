 ...
 * @param[in]  num - qdec module number,QDEC0 or QDEC1.
 * @param[in]  chn_a - input types of a channel.
 * @param[in]  chn_b - input types of b channel.
 * @return     none.
 */
void qdec_set_pin(qdec_num_e num, qdec_chn_e chn_a, qdec_chn_e chn_b)
{
    reg_qdec_channel_a(num) = chn_a;
    reg_qdec_channel_b(num) = chn_b;
}
 ...
 * @return     none.
 */
void qdec_set_mode(qdec_num_e num, qdec_mode_e mode)
{
    if (mode == COMMON_MODE) {
        reg_qdec_mode(num) &= (~FLD_QDEC_DOUBLE0);
 ...
 * @return     none.
 */
void qdec_clk_en(qdec_num_e num)
{
    clock_32k_init(CLK_32K_RC);
    clock_cal_32k_rc();
    if (num == QDEC0) {
        reg_clk_en3 |= FLD_CLK3_QDEC0_EN;
    } else if (num == QDEC1) {
        reg_clk_en3 |= FLD_CLK3_QDEC1_EN;
    }
    qdec_reset(num);
}
 ...
 * @return     none.
 */
void qdec_reset(qdec_num_e num)
{
    if (num == QDEC0) {
        reg_rst3 &= ~(FLD_RST3_QDEC);
        reg_rst3 |= FLD_RST3_QDEC;
    } else if (num == QDEC1) {
        reg_rst3 &= ~(FLD_RST3_QDEC1);
        reg_rst3 |= FLD_RST3_QDEC1;
    }
}
 ...
 * @return     none.
 */
void qdec_set_debouncing(qdec_num_e num, qdec_thrsh_e thrsh)
{
    reg_qdec_dbntime(num) &= (~FLD_QDEC_DBNTIME);
    reg_qdec_dbntime(num) |= thrsh;
}
