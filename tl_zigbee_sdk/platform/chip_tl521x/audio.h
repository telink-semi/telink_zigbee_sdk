...
typedef enum
{
    ACLK_DBG_OPEN = BIT(0), //audio core clock 24m
    SDM_CLK_DBG   = BIT(1), //sdm clock
    I2S_CLK_DBG   = BIT(3), //i2s mclock
    ADC_CLK6M_DBG = BIT(4), //codec adc sample clock
    ADC_CLK1M_DBG = BIT(5), //codec adc core clock
    CODEC_CLK_DBG = BIT(6), //audio codec clock
} audio_clk_dbg_e;
...
typedef enum
{
    ...
    MEDIUM_POWER_CONSUMPTION   = 1,
    LOW_POWER_CONSUMPTION      = 2,
    AUDIO_ADC_POWER_DOWN       = 3,
}audio_adc_power_mode_e;
 ...
 */
static inline void audio_set_audio_clk(unsigned short div_numerator, unsigned short div_denominator)
{
...
{
    BM_SET(reg_rx_wptr_en, fifo_chn);
}
...
{
    BM_CLR(reg_rx_wptr_en, fifo_chn);
}
...
{
    analog_write_reg8(areg_0x10d, ((analog_read_reg8(areg_0x10d) & (~FLD_L_PGA_VOL)) | MASK_VAL(FLD_L_PGA_VOL, pga_gain)));
}
...
{
    codec_in_pga_gain_e val = CODEC_IN_GAIN_0_DB;
    val = (analog_read_reg8(areg_0x10d) & FLD_L_PGA_VOL) >> 4;
    return val;
}
...
/**
 *  @brief      This function serves to clear input path gain.
 *  @return     none.
 *  @note       Due to the -16.74dB gain in the codec path after shifting the data left by 1 bit,
 *              it is necessary to amplify the input by a factor of 7/-7 to eliminate the gain.
 */
static inline void audio_codec_stream0_path_gain_clr(void)
{
    reg_codec_rst |= MASK_VAL(FLD_SFT_ZC, 1); /* Shifting the data left by 1 bit increases the gain by 6dB. */
    reg_dmic_dat = MASK_VAL(FLD_DMIC_DAT_POS, 7) | MASK_VAL(FLD_DMIC_DAT_NEG, 9); /* 9 means -7 here */
}
...
/**
 *  @brief      This function serves to set sdm clock.
 *  @param[in]  div_numerator   - the dividing factor of div_numerator (15bits valid).
 *  @param[in]  div_denominator - the dividing factor of div_denominator(16bits valid).
 *  @return     none.
 */
 ...
 *  @return     none.
 */
static inline void audio_set_sdm_src_clk_en(void)
{
 ...
 *  @return     none.
 */
static inline void audio_set_sdm_src_clk_dis(void)
{
    reg_sdm_step &= ~FLD_SDM_CLK_EN_0;
}
...
{
    reg_i2s2_step = (div_numerator & FLD_I2S2_STEP) | FLD_I2S2_EN;
...
/**
 *  @brief      This function serves to ascl line enable
 *  @param[in]  ascl_select - ascl channel
 *  @return     none
 */
static inline void audio_set_ascl_en(audio_ascl_select_e ascl_select)
{
    if (ascl_select != ASCL0)
    {
        BM_SET(reg_line(ascl_select), FLD_LINE_0);
    }
    else
    {
        BM_SET(reg_aud_ctrl, FLD_LINE_0);
...
{
    (void) i2s_select;
    reg_i2s_2fifo_mode |= FLD_I2S_2FIFO_MODE;
}
 ...
 * @param[in]  mode - audio_adc_power_mode_e.
 * @return     none.
 */
void audio_codec_set_adc_power_mode(audio_adc_power_mode_e mode);
...
/**
 * @brief     This function configures the i2s clk to the external codec mclk via the debug pin,mclk=144M*(div_numerator/div_denominator)
 * @param[in] mclk_pin -mclk output pin.
 * @param[in] div_numerator   - the dividing factor of div_numerator (15bits valid).
 * @param[in] div_denominator - the dividing factor of div_denominator(16bits valid).
 * @return    none.
...
/**
 * @brief     This function configures the internal codec clk to the external codec mclk via the debug pin,mclk=144M*(div_numerator/div_denominator):144M*(1/12)=12M.
 * @param[in] mclk_pin -mclk output pin.
 * @return    none.
