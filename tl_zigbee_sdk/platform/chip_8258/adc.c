...
#define ADC_SAMPLE_NUM          8//4

/**
 * @brief This function serves to set adc sampling and get results.
 * @param[in]  mode--whether calculate difference between maximum and minimum voltage values.
 * @param[in]  d -- the difference between maximum and minimum voltage values.
 * @return the result of sampling.
 */
_attribute_ram_code_ unsigned int adc_sample_and_get_result_op(unsigned char mode, unsigned int *d)
{
    unsigned short temp;
...
unsigned int adc_sample_and_get_result_manual(void)
{
    unsigned short v = adc_sample_and_get_result_manual_mode();

    unsigned short adc_result = ((v * adc_pre_scale * adc_vref) >> 13) + adc_vref_offset;
