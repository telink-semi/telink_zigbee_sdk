...
#ifdef ZCL_IDENTIFY
static s32 sampleGW_zclIdentifyTimerCb(void *arg)
{
...
static void sampleGW_zclIdentifyTimerStop(void)
{
    if (identifyTimerEvt) {
...
#if ZB_TEST_ENABLE
    zcl_identifyRspCmd_t *identifyRsp = (zcl_identifyRspCmd_t *)cmdPayload;
    if (identifyRsp->timeout) {
        findBindDst_t dstInfo;
        dstInfo.addr = pAddrInfo->srcAddr;
        dstInfo.endpoint = pAddrInfo->dstEp;
