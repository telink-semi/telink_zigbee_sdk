...
const zdo_appIndCb_t appCbLst = {
    ...
    NULL,                               //nwk panid conflict ind cb
    NULL,                               //permit join ind cb
    NULL,                               //nlme sync cnf cb
    NULL,                               //tc join ind cb
    NULL,                               //tc detects that the frame counter is near limit
    NULL,                               //end device negotiate cnf cb
 ...
 */
drv_pm_pinCfg_t g_switchPmCfg[] = BUTTON_WAKEUP_CFG;
...
#if PM_ENABLE && (!BLE_CONCURRENT_MODE)
...
#if ZBHCI_EN
#if ZB_TEST_ENABLE
    zbhciInit(zbhciProcessIncomingSerialCmd);
#else
    zbhciInit(zbhciCmdHandler);
#endif
