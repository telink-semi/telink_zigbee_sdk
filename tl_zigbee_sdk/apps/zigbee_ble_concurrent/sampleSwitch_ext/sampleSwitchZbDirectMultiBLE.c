/********************************************************************************************************
 * @file    sampleSwitchDirectMultiBLE.c
 *
 * @brief   This is the source file for sampleSwitchDirectMultiBLE
 *
 * @author  Zigbee Group
 * @date    2024
 *
...
_attribute_data_retention_ u32 flash_sector_mac_address = FLASH_ADDR_OF_MAC_ADDR_1M;
...
const u8 tbl_advData[] = {
     ...
     0x03, DT_INCOMPLETE_LIST_16BIT_SERVICE_UUID, 0xF7, 0xFF,             // commissioning service UUID
};
...
static const u8 my_devName[] = {'m','u','l','t','i','-','-','s','w','i','t','c','h'};
...
u8 my_commissChangeCCC[2] = {01, 01};
...
void app_zbd_advDataSet(void)
{
...
#endif
    {
        ...
        zbd_extendAdvData(advDataExt, (u8*)&(advLen), 0);
...
static const attribute_t my_Attributes[] = {
    ...
    // 001a - 0029
    {16,ATT_PERMISSIONS_READ,2,2,(u8*)(size_t)(&my_primaryServiceUUID), (u8*)(size_t)(&my_commServiceUUID), 0, 0},
...
#ifdef ZCL_ZBD_CONFIG
            extern void sampleSwitchAnJoinTimerStart(void);
...
void zbd_bdbcommissioningCb(u8 status)
{
    ...
    if (status != BDB_COMMISSION_STA_NO_IDENTIFY_QUERY_RESPONSE) {
        app_zbd_commissioningResultNotify(status);
    }
...
void app_process_power_management(void)
{
    ...
    if (tlkapi_debug_isBusy()) {
        ...
        if ((g_switchAppCtx.state == APP_KEY_RELEASED) &&
