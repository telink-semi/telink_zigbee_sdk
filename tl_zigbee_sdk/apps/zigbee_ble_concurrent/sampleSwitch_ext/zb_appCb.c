...
extern void zbd_bdbInitCb(u8 status, u8 joinedNetwork);
extern void zbd_bdbcommissioningCb(u8 status);
extern int app_zbd_zvdBleDisconnect(u8 ret);
#endif
/**********************************************************************
 * LOCAL FUNCTIONS
 */
 ...
 */
#if (!ZIGBEE_DIRECT_ENABLE)
static s32 sampleSwitch_bdbNetworkSteerStart(void *arg)
{
...
static s32 sampleSwitch_bdbFindAndBindStart(void *arg)
{
...
static s32 sampleSwitch_rejoinBackoff(void *arg)
{
...
#if ZIGBEE_DIRECT_ENABLE
    zbd_bdbInitCb(status, joinedNetwork);
#endif
    ...
    if (status == BDB_INIT_STATUS_SUCCESS) {
        ...
        if (joinedNetwork) {
            ...
            if (!rejoinBackoffTimerEvt) {
                rejoinBackoffTimerEvt = TL_ZB_TIMER_SCHEDULE(sampleSwitch_rejoinBackoff, NULL, 60 * 1000);
            }
        }
    }
}
...
void zbdemo_bdbCommissioningCb(u8 status, void *arg)
{
    ...
    case BDB_COMMISSION_STA_SUCCESS:
        ...
        ota_queryStart(OTA_PERIODIC_QUERY_INTERVAL);
#endif
#if (FIND_AND_BIND_SUPPORT && (!ZIGBEE_DIRECT_ENABLE))
...
s32 sampleSwitch_Reset(void *arg)
{
#if ZIGBEE_DIRECT_ENABLE
    //6.2.3
    app_zbd_zvdBleDisconnect(1);    //disconnect with ZVD
#else
    SYSTEM_RESET();
#endif
...
void sampleSwitch_leaveCnfHandler(nlme_leave_cnf_t *pLeaveCnf)
{
    if (pLeaveCnf->status == SUCCESS) {
        ...
        //waiting blink over
        TL_ZB_TIMER_SCHEDULE(sampleSwitch_Reset, NULL, 3 * 1000);
    }
}
...
/*********************************************************************
 * @fn      sampleSwitch_nwkStatusIndHandler
 *
