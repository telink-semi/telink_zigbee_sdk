/********************************************************************************************************
 * @file    sampleSwitchMultiBLE.c
 *
 * @brief   This is the source file for sampleSwitchMultiBLE
 *
 * @author  Zigbee Group
 * @date    2024
 *
...
//////////////////////////// User Configuration for BLE application ////////////////////////////
    ...
    blc_pm_setSleepMask(PM_SLEEP_LEG_ADV | PM_SLEEP_LEG_SCAN | PM_SLEEP_ACL_PERIPHR | PM_SLEEP_ACL_CENTRAL);
        ...
        blc_pm_setDeepsleepRetentionThreshold(95);
            ...
            blc_pm_setDeepsleepRetentionType(DEEPSLEEP_MODE_RET_SRAM_LOW96K);
                ...
                blc_pm_setDeepsleepRetentionEarlyWakeupTiming(1980); //enable voltage detect
            #else
                blc_pm_setDeepsleepRetentionEarlyWakeupTiming(1180); //for tl521x 72M
            #endif
            blc_pm_setDeepsleepRetentionType(DEEPSLEEP_MODE_RET_SRAM_LOW128K);
        #endif
    #else
    ...
    blc_ota_setProgramOffset(newAddr);
    blc_ota_setFirmwareSizeAndBootAddress(FLASH_OTA_IMAGE_MAX_SIZE, FLASH_ADDR_OF_OTA_IMAGE);

    /*it will clear the old firmware in ble stack.*/
    #if FLASH_PROTECT_ENABLE
        flash_unlock();
    #endif

    blc_ota_initOtaServer_module();
    blc_ota_setOtaProcessTimeout(800);//adjust according to the conn interval and the firmware size by user
...
void app_process_power_management(void)
{
    ...
    if (tlkapi_debug_isBusy()) {
        ...
        int user_task_flg = g_switchAppCtx.keyPressedCnt;
